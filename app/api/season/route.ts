...
function seedWeightsFromWins(wins: number): number[] {
  if (wins >= 64) return [0.78, 0.14, 0.05, 0.02, 0.01, 0,    0,    0];
  if (wins >= 57) return [0.50, 0.28, 0.12, 0.06, 0.03, 0.01, 0,    0];
  if (wins >= 50) return [0.15, 0.25, 0.25, 0.18, 0.10, 0.05, 0.02, 0];
  if (wins >= 44) return [0.04, 0.08, 0.12, 0.18, 0.22, 0.18, 0.12, 0.06];
